...
const user = { id: 'user-1', must_change_password: 0 };
...
test('create handler stores server risk and completed structured reading', async () => {
  ...
  const response = await handleCreateReading({ request: request(), env: { OPENAI_API_KEY: 'key' }, repository, ai, user });
...
test('create handler ignores client risk and reclassifies professional questions', async () => {
  ...
  await handleCreateReading({ request: request({ ...payload, question: { ...payload.question, text: '这笔股票投资要不要全部买入？' } }), env: { OPENAI_API_KEY: 'key' }, repository, ai: async () => ({ status: 'completed', reading: {} }), user });
  assert.equal(serverRisk.level, 'high');
  assert.ok(serverRisk.categories.includes('investment'));
});

test('retry requires the same authenticated user', async () => {
  const repository = { getForUser: async (_id, userId) => userId === user.id ? { id: 'reading-1', payload, risk_level: 'normal', riskCategories: [], status: 'failed' } : null, markProcessing: async () => {}, complete: async () => {}, addAttempt: async () => {} };
  const response = await handleRetryReading({ request: new Request('https://example.com/retry', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ deviceId: payload.deviceId }) }), env: { OPENAI_API_KEY: 'key' }, readingId: 'reading-1', repository, ai: async () => ({ status: 'completed', reading: { overall_judgment: '完成' } }), user });
...
test('retry force-regenerates a completed legacy reading', async () => {
  let aiCalls = 0;
  const repository = {
    getForUser: async () => ({ id: 'reading-legacy', payload, risk_level: 'normal', riskCategories: [], status: 'completed', aiReading: { overall_judgment: '旧版结果' } }),
  ...
  const response = await handleRetryReading({
    ...
    ai: async () => { aiCalls += 1; return { status: 'completed', reading: { comprehensive_hexagram_reading: { conclusions: ['新版结果'] } } }; }, user
  });
...
test('concurrent idempotent create returns the row inserted by another request', async () => {
  ...
  const response = await handleCreateReading({ request: request(), env: { OPENAI_API_KEY: 'key' }, repository, ai: async () => { throw new Error('should not call'); }, user });
  assert.equal(response.status, 200);
...
test('create handler passes configured relay settings to the AI client', async () => {
  ...
  await handleCreateReading({
    ...
    repository,
    ai: async (input) => { aiInput = input; return { status: 'completed', reading: {} }; }, user
  });
...
test('retry handler passes configured relay settings to the AI client', async () => {
  let aiInput;
  const repository = { getForUser: async () => ({ id: 'relay-retry', payload, risk_level: 'normal', riskCategories: [], status: 'failed' }), markProcessing: async () => {}, complete: async () => {}, addAttempt: async () => {} };
