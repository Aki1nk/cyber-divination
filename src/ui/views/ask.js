      ...
      <form class="casting-form" data-casting-form>
        <fieldset class="form-section"><legend><span>壹</span> 定问</legend>
          ...
          <div class="field-grid">
            ...
            <label>背景补充<input name="background" maxlength="200" placeholder="选填，仅存本机"></label>
          </div>
        </fieldset>
        ...
        <fieldset class="form-section"><legend><span>叁</span> 取数</legend>
          ...
          <div class="method-panel" data-method-panel="digital-symbol" hidden><p class="method-notice"><strong>现代数字取象法</strong>此法并非古籍原法。点击起卦时，本机将调用安全随机源三次，不联网、不上传。</p></div>
          ${renderTimeFields()}
          ${renderExternalFields()}
        </fieldset>
        <section class="confirmation-card"><strong>肆 · 确认</strong><p>提交后将生成不可变计算快照。同一问题 24 小时内再次占问，将返回原卦以避免反复占问。</p><button class="primary-action primary-action--button" type="submit"><span>确认起卦</span><small>生成本卦 · 互卦 · 变卦</small></button><p class="form-status" role="status" data-form-status></p></section>
      </form>
    </section>`;
}
